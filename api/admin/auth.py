...
admin_auth_namespace = Namespace('admin/auth', description='Admins registration and authentication operations')

signup_model = admin_auth_namespace.model('Signup', {
    "id": fields.Integer(),
...
user_model = admin_auth_namespace.model('User', {
    "id": fields.Integer(),
...
login_model = admin_auth_namespace.model('Login', {
    "email": fields.String(required=True, description='The email address'),
    "password": fields.String(required=True, description='The password')
})

@admin_auth_namespace.route('/register')
class Register(Resource):
    @admin_auth_namespace.expect(signup_model)
    @admin_auth_namespace.marshal_with(user_model)
    @admin_auth_namespace.doc(description="Register a new user")
    def post(self):
        """
            Create a new user
        """
        ...
        try:
            new_user = Admin(username=data['username'], email=data['email'])
...
class Login(Resource):
    @admin_auth_namespace.expect(login_model)
    @admin_auth_namespace.doc(description="Login a user to get the JWT access and refresh tokens")
    def post(self):
        """
...
@admin_auth_namespace.route('/refresh')
class Refresh(Resource):
    @jwt_required(refresh=True)
    @admin_auth_namespace.doc(description="Refresh a user's expired JWT access token")
    def post(self):
        """
...
@admin_auth_namespace.route('/whoami')
class WhoAmI(Resource):
    @jwt_required()
    @admin_auth_namespace.doc(description="Get the user's details")
    def get(self):
        """
